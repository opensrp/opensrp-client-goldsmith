...
import org.smartregister.goldsmith.dao.ChwPNCDaoFlv;
...
public class BaPncHomeVisitInteractorFlv extends DefaultPncHomeVisitInteractorFlv {
        ...
        HomeVisitActionHelper helper = new HomeVisitActionHelper() {
            ...
            public void onPayloadReceived(String jsonPayload) {
                try {
                    ...
                    fp_period_received = JsonFormUtils.getValue(jsonObject, "fp_period_received");
    ...
    private void evaluateObsIllnessMother() throws Exception {
        HomeVisitActionHelper obsIllnessMotherHelper = new HomeVisitActionHelper() {
            ...
            public void onPayloadReceived(String jsonPayload) {
                try {
                    JSONObject jsonObject = new JSONObject(jsonPayload);
                    date_of_illness = JsonFormUtils.getValue(jsonObject, "date_of_illness_mama");
                    illness_description = JsonFormUtils.getValue(jsonObject, "illness_description_mama");
                    action_taken = JsonFormUtils.getCheckBoxValue(jsonObject, "action_taken_mama");
    ...
    private void evaluateObsIllnessBaby(Person baby) throws Exception {
        class ObsIllnessBabyHelper extends HomeVisitActionHelper {
            ...
            public void onPayloadReceived(String jsonPayload) {
                try {
                    JSONObject jsonObject = new JSONObject(jsonPayload);
                    date_of_illness = JsonFormUtils.getValue(jsonObject, "date_of_illness_child");
                    illness_description = JsonFormUtils.getValue(jsonObject, "illness_description_child");
                    action_taken = JsonFormUtils.getCheckBoxValue(jsonObject, "action_taken_child");
                    illnessDate = DateTimeFormat.forPattern("dd-MM-yyyy").parseLocalDate(date_of_illness);
                } catch (Exception e) {
                    Timber.e(e);
                }
    ...
    private class PNCHealthFacilityVisitHelper implements BaseAncHomeVisitAction.AncHomeVisitActionHelper {
        ...
        public void onPayloadReceived(String jsonPayload) {
            try {
                JSONObject jsonObject = new JSONObject(jsonPayload);
                pnc_visit = JsonFormUtils.getValue(jsonObject, MessageFormat.format("pnc_visit_{0}", visit_num));
                pnc_hf_visit_date = JsonFormUtils.getValue(jsonObject, MessageFormat.format("pnc_hf_visit{0}_date", visit_num));

                if (visit_num == 1) {
                    vit_a_mother = JsonFormUtils.getValue(jsonObject, "vit_a_mother");
                    ifa_mother = JsonFormUtils.getValue(jsonObject, "ifa_mother");
                }
        ...
        public String postProcess(String s) {
            try {
                ...
                String value = JsonFormUtils.getValue(jsonObject, MessageFormat.format("pnc_visit_{0}", visit_num));
